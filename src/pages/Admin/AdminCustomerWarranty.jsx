                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (String(deleteRetailOrder.id).startsWith('RTL-DB-')) {
                        ...
                        const response = await fetch(`http://localhost:5000/api/transactions/${dbId}`, {
                          method: 'DELETE',
                          headers: {
                        ...
                        {historyItem.isComplaint && (
                          <div style={{ marginTop: '12px', padding: '12px', backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #ffe4e6' }}>
                            ...
                            {historyItem.proof_photo && (
                              <img 
                                src={historyItem.proof_photo.startsWith('http') ? historyItem.proof_photo : `http://localhost:5000${historyItem.proof_photo}`} 
                                alt="Bukti Komplain" 
                                style={{ maxWidth: '80px', borderRadius: '6px', cursor: 'pointer', border: '1px solid #fecaca', display: 'block' }} 
                                onClick={() => window.open(historyItem.proof_photo.startsWith('http') ? historyItem.proof_photo : `http://localhost:5000${historyItem.proof_photo}`, '_blank')} 
                              />
                            )}
                          </div>
                        )}
